...
class RuleSimulator(UserSimulator):
    ...
    def __init__(self, selfplay, movie_dict=None, act_set=None, slot_set=None, params=None):
        ...
        self.print_training_utt = params['print_training_mode']
        self.print_testing_utt = params['print_testing_mode']
        self.utterance_save_path = params['utterance_save_path']
    ...
    def _sample_action(self):
        ...
        dict_key_list = list(dialog_config.start_dia_acts.keys())
        self.state['diaact'] = random.choice(dict_key_list)
        ...
        if (self.print_training_utt and self.phase == "training") or (self.print_testing_utt and self.phase == "testing"):
            sentence = self.add_nl_to_action(sample_action)
            if self.phase == "training":
                path = self.utterance_save_path + "/resultant_training_dialogues.txt"
            else:
                path = self.utterance_save_path + "/resultant_testing_dialogues.txt"
            with open(path, "a") as f:
                f.write(sentence + "\ndiaact : " + str(sample_action['diaact']) +
                        "\nrequest_slots :" + str(sample_action['request_slots']) +
                        "\ninform_slots : " + str(sample_action['inform_slots']) + "\n\n")
    ...
    def next(self, system_action):
        ...
        # add NL to dia_act
        if (self.print_training_utt and self.phase == "training") or (self.print_testing_utt and self.phase == "testing"):
            sentence = self.add_nl_to_action(response_action)
            if self.phase == "training":
                path = self.utterance_save_path + "/resultant_training_dialogues.txt"
            else:
                path = self.utterance_save_path + "/resultant_testing_dialogues.txt"
            with open(path, "a") as f:
                f.write(sentence + "\ndiaact : " + str(response_action['diaact']) +
                        "\nrequest_slots :" + str(response_action['request_slots']) +
                        "\ninform_slots : " + str(response_action['inform_slots']) + "\n\n")
