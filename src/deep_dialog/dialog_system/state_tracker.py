...
class StateTracker:
    ...
    def initialize_episode(self):
        ...
        self.turn_count = 0
        self.current_slots = {}
        self.essential_slots = {'date', 'moviename', 'numberofpeople', 'starttime', 'theater'}
        self.end_conv = False
    ...
    def update(self, agent_action=None, user_action=None):
        ...
        if agent_action:
            if agent_action != None and 'theater' in agent_action['act_slot_response']['inform_slots']: 
                agent_action['act_slot_response']['inform_slots']['city'] = 'PLACEHOLDER'
        ...
        ########################################################################
            ...
            if 'theater' in user_action['inform_slots']:
                if 'city' in self.current_slots['inform_slots']:
                    self.current_slots['inform_slots']['city'] = 'PLACEHOLDER'
                if 'city' in self.current_slots['proposed_slots']:
                    del self.current_slots['proposed_slots']['city']
            ...
            if (user_action['diaact'] == 'request' and user_action['inform_slots'] == {} and user_action['request_slots'] == {'ticket':'UNK'}  and self.current_slots["request_slots"] == {'ticket':'UNK'}) or (user_action['diaact'] == 'inform' and (self.current_slots["request_slots"] == {'ticket':'UNK'} or self.current_slots["request_slots"] == {})):
                complete = True
                for slot in self.essential_slots:
                    if slot not in self.current_slots['inform_slots'] : 
                        complete = False
                    else:
                        if self.current_slots['inform_slots'][slot] == 'NO VALUE MATCHES!!!':
                            complete = False
                if complete:
                    self.end_conv = True
                    self.current_slots['inform_slots']['taskcomplete']= 'Ticket Available'
                    self.current_slots['proposed_slots']['taskcomplete']= 'Ticket Available'


            #if user_action['diaact'] =='inform' and user_action['inform_slots'] == {} and user_action['request_slots'] == {}:
            #    self.current_slots['proposed_slots'] in 
...
movie_kb = pickle.load(open('/home/thisara/Documents/FYP/Simulator_selfplay-master/src/deep_dialog/data/movie_kb.1k.p', 'rb'), encoding='utf-8')
act_set = text_to_dict('/home/thisara/Documents/FYP/Simulator_selfplay-master/src/deep_dialog/data/dia_acts.txt')
slot_set = text_to_dict('/home/thisara/Documents/FYP/Simulator_selfplay-master/src/deep_dialog/data/slot_set.txt')

st = StateTracker(act_set, slot_set, movie_kb)
st.initialize_episode()
k = st.update(agent_action= {'act_slot_response': {'diaact': 'inform', 'inform_slots': {'starttime': 'PLACEHOLDER'}, 'request_slots': {}}, 'act_slot_value_response': None})
print(k)
"""
