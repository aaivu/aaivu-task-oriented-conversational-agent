...
class DialogManager:
    ...
    def next_turn(self, record_training_data=True):
        ...
        if (self.user.print_training_utt and self.user.phase == "training") or (self.user.print_testing_utt and self.user.phase == "testing"):
            sentence = self.agent.add_nl_to_action(self.agent_action) # add NL to Agent Dia_Act
            if self.user.phase == "training":
                path = self.user.utterance_save_path + "/resultant_training_dialogues.txt"
            else:
                path = self.user.utterance_save_path + "/resultant_testing_dialogues.txt"
            with open(path, "a") as f:
                f.write(sentence + "\ndiaact : " + str(self.agent_action['act_slot_response']['diaact']) +
                        "\nrequest_slots :" + str(self.agent_action['act_slot_response']['request_slots']) +
                        "\ninform_slots : " + str(self.agent_action['act_slot_response']['inform_slots']) + "\n\n")
